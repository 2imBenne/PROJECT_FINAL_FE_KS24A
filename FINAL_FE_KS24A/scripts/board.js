...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function setupKanbanEvents(boardId) {
    ...
    // ======= DATES MODAL (Figma style, chỉ ngày tháng năm) ======= //
    // State for calendar
    let calendarState = {
      viewDate: new Date(),
      selected: {
      ...
      activeType: 'start' // 'start' | 'due'
    };

    function openDatesModal(card) {
      calendarState.selected.start = card.startDate ? parseDate(card.startDate) : null;
      calendarState.selected.due = card.dueDate ? parseDate(card.dueDate) : null;
      calendarState.activeType = 'start';
      calendarState.viewDate = (calendarState.selected.start || calendarState.selected.due || new Date());
      setDatesModalUI();
      renderCalendar();
      document.getElementById('datesModal').style.display = 'flex';
    }

    function setDatesModalUI() {
      const startCheck = document.getElementById('startDateCheck');
      const startInput = document.getElementById('startDateInput');
      if (calendarState.selected.start) {
        startCheck.checked = true;
        startInput.value = formatDate(calendarState.selected.start);
      ...
      const dueCheck = document.getElementById('dueDateCheck');
      const dueInput = document.getElementById('dueDateInput');
      if (calendarState.selected.due) {
        dueCheck.checked = true;
        dueInput.value = formatDate(calendarState.selected.due);
    ...
    function renderCalendar() {
      ...
      for (let i = 0; i < startDay; i++) {
        const d = prevMonth.getDate() - startDay + i + 1;
      ...
      for (let d = 1; d <= totalDays; d++) {
        ...
        if (calendarState.activeType === 'start' && calendarState.selected.start && isSameDay(thisDate, calendarState.selected.start)) el.classList.add('selected');
        if (calendarState.activeType === 'due' && calendarState.selected.due && isSameDay(thisDate, calendarState.selected.due)) el.classList.add('selected');
        if (isToday(thisDate)) el.classList.add('today');
        el.onclick = () => {
          if (calendarState.activeType === 'start') {
            calendarState.selected.start = thisDate;
          } else {
            calendarState.selected.due = thisDate;
          }
          setDatesModalUI();
          renderCalendar();
        };
      ...
      let after = 42 - (startDay + totalDays);
    ...
    const startCheck = document.getElementById('startDateCheck');
    const dueCheck = document.getElementById('dueDateCheck');
    if (startCheck && dueCheck) {
      startCheck.onchange = () => {
        calendarState.activeType = 'start';
        if (!startCheck.checked) calendarState.selected.start = null;
        setDatesModalUI();
        renderCalendar();
      };
      dueCheck.onchange = () => {
        calendarState.activeType = 'due';
        if (!dueCheck.checked) calendarState.selected.due = null;
        setDatesModalUI();
        renderCalendar();
      };
    }
    ...
    if (startInput && dueInput) {
      startInput.onfocus = () => { calendarState.activeType = 'start'; renderCalendar(); };
      dueInput.onfocus = () => { calendarState.activeType = 'due'; renderCalendar(); };
    }
    const saveBtn = document.querySelector('.save-dates');
    if (saveBtn) {
      saveBtn.onclick = () => {
        const modal = document.getElementById('datesModal');
        const cardId = modal.dataset.cardId;
        const listId = document.getElementById('modalTaskDetail').getAttribute('data-list-id');
          ...
          if (list.id === listId) {
            list.cards = list.cards.map(card => {
              if (card.id === cardId) {
                return {
                  ...card,
                  startDate: startCheck.checked && calendarState.selected.start ? formatDate(calendarState.selected.start) : undefined,
                  dueDate: dueCheck.checked && calendarState.selected.due ? formatDate(calendarState.selected.due) : undefined
                };
              }
              return card;
            });
    ...
    const removeBtn = document.querySelector('.remove-dates');
    if (removeBtn) {
      removeBtn.onclick = () => {
        const modal = document.getElementById('datesModal');
        const cardId = modal.dataset.cardId;
    ...
    const closeBtn = document.querySelector('#datesModal .close-modal');
    if (closeBtn) closeBtn.onclick = () => {
      document.getElementById('datesModal').style.display = 'none';
    };
    const datesBtn = document.querySelector('.task-detail-dates');
    if (datesBtn) {
      datesBtn.onclick = function(e) {
        e.stopPropagation();
        const modal = document.getElementById('modalTaskDetail');
        const listId = modal.getAttribute('data-list-id');
        const cardId = modal.getAttribute('data-card-id');
        ...
        const list = lists.find(l => l.id === listId);
        const card = list.cards.find(c => c.id === cardId);
        document.getElementById('datesModal').dataset.cardId = cardId;
        openDatesModal(card);
      };
    }
  }
  ...
  // ===== LABELS MODAL LOGIC =====
  const LABEL_COLORS = [
    '#B2F2D7', '#FFE066', '#FFD6A5', '#B5EAD7', '#C7CEEA',
    '#76E2B3', '#FFD166', '#FFB3B3', '#FFB7B2', '#B5A7F7'
  ];

  function getLabels(boardId) {
    return JSON.parse(localStorage.getItem(`labels_${boardId}`) || '[]');
  }
  function saveLabels(boardId, labels) {
    localStorage.setItem(`labels_${boardId}`, JSON.stringify(labels));
  }

  // Open labels modal from task detail
  const labelBtn = document.querySelector('.task-detail-labels');
  if (labelBtn) {
    labelBtn.onclick = function() {
      const boardId = new URLSearchParams(window.location.search).get('id');
      renderLabelsModal(boardId);
      document.getElementById('labelsModal').style.display = 'flex';
    };
  }

  // Render danh sách label
  function renderLabelsModal(boardId) {
    const labels = getLabels(boardId);
    const modal = document.getElementById('labelsModal');
    const listDiv = document.getElementById('labelsList');
    listDiv.innerHTML = '';
    // Lấy card hiện tại
    const cardId = document.getElementById('modalTaskDetail').getAttribute('data-card-id');
    const listId = document.getElementById('modalTaskDetail').getAttribute('data-list-id');
    let lists = getBoardData(boardId);
    const list = lists.find(l => l.id === listId);
    const card = list.cards.find(c => c.id === cardId);
    ...
    labels.forEach(label => {
      ...
      cb.type = 'checkbox';
      cb.className = 'label-checkbox';
      cb.checked = cardLabels.includes(label.id);
      cb.onchange = function() {
        // Toggle label cho card
        let lists = getBoardData(boardId);
        lists = lists.map(l => {
          if (l.id === listId) {
            l.cards = l.cards.map(c => {
              if (c.id === cardId) {
                let newLabels = c.labels || [];
                if (cb.checked) {
                  if (!newLabels.includes(label.id)) newLabels.push(label.id);
                } else {
                  newLabels = newLabels.filter(id => id !== label.id);
                }
      ...
      editBtn.onclick = function() {
        openEditLabelModal(label, boardId);
      };
  ...
  const btnShowCreateLabel = document.getElementById('btnShowCreateLabel');
  if (btnShowCreateLabel) {
    btnShowCreateLabel.onclick = function() {
      document.getElementById('labelsModal').style.display = 'none';
      renderLabelColorPalette('createLabelColors', null);
      document.getElementById('createLabelTitle').value = '';
      document.getElementById('createLabelModal').style.display = 'flex';
    };
  }
  // Back to list from create
  const createBackBtn = document.querySelector('#createLabelModal .back-to-labels');
  if (createBackBtn) {
    createBackBtn.onclick = function() {
      document.getElementById('createLabelModal').style.display = 'none';
      document.getElementById('labelsModal').style.display = 'flex';
    };
  }
  // Back to list from edit
  const editBackBtn = document.querySelector('#editLabelModal .back-to-labels');
  if (editBackBtn) {
  ...
  // Close all label modals
  Array.from(document.querySelectorAll('.close-labels-modal')).forEach(btn => {
  ...
  if (btnCreateLabel) {
    btnCreateLabel.onclick = function() {
      const boardId = new URLSearchParams(window.location.search).get('id');
      const title = document.getElementById('createLabelTitle').value.trim();
      const color = getSelectedColor('createLabelColors');
      if (!title || !color) return;
      let labels = getLabels(boardId);
      const id = 'label_' + Date.now() + '_' + Math.floor(Math.random()*10000);
      labels.push({ id, title, color });
      saveLabels(boardId, labels);
      document.getElementById('createLabelModal').style.display = 'none';
      renderLabelsModal(boardId);
      document.getElementById('labelsModal').style.display = 'flex';
    };
  ...
  // Edit label
  function openEditLabelModal(label, boardId) {
    document.getElementById('labelsModal').style.display = 'none';
    document.getElementById('editLabelTitle').value = label.title;
    renderLabelColorPalette('editLabelColors', label.color);
    document.getElementById('editLabelModal').style.display = 'flex';
    document.getElementById('editLabelModal').setAttribute('data-label-id', label.id);
  }
  const btnSaveLabel = document.getElementById('btnSaveLabel');
  if (btnSaveLabel) {
    btnSaveLabel.onclick = function() {
      const boardId = new URLSearchParams(window.location.search).get('id');
      const id = document.getElementById('editLabelModal').getAttribute('data-label-id');
      const title = document.getElementById('editLabelTitle').value.trim();
      const color = getSelectedColor('editLabelColors');
      if (!title || !color) return;
      let labels = getLabels(boardId);
      labels = labels.map(l => l.id === id ? { ...l, title, color } : l);
      saveLabels(boardId, labels);
      document.getElementById('editLabelModal').style.display = 'none';
      renderLabelsModal(boardId);
      document.getElementById('labelsModal').style.display = 'flex';
  ...
  if (btnDeleteLabel) {
    btnDeleteLabel.onclick = function() {
      const boardId = new URLSearchParams(window.location.search).get('id');
      const id = document.getElementById('editLabelModal').getAttribute('data-label-id');
      let labels = getLabels(boardId);
      labels = labels.filter(l => l.id !== id);
      saveLabels(boardId, labels);
      // Xóa label khỏi tất cả card
      let lists = getBoardData(boardId);
      lists = lists.map(list => ({
        ...list,
        cards: list.cards.map(card => ({
  ...
  function renderLabelColorPalette(containerId, selectedColor) {
    const container = document.getElementById(containerId);
    container.innerHTML = '';
    LABEL_COLORS.forEach(color => {
      const btn = document.createElement('button');
      btn.className = 'label-color-btn' + (color === selectedColor ? ' selected' : '');
      ...
      if (color === selectedColor) {
        btn.innerHTML = '<span class="color-check">&#10003;</span>';
      }
      container.appendChild(btn);
    });
  }
  function getSelectedColor(containerId) {
    const btn = document.querySelector(`#${containerId} .label-color-btn.selected`);
