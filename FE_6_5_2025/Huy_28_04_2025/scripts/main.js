...
function setupModalSelectEvents() {
    document.querySelectorAll('.modal-board-bg-option').forEach(btn => {
        btn.addEventListener('click', function() {
            // Remove selected class from all background and color options
            document.querySelectorAll('.modal-board-bg-option, .modal-board-color-option').forEach(b => b.classList.remove('selected'));
...
function getCreateModalData() {
    ...
    // Get the first background option as default
    const defaultBgBtn = document.querySelector('.modal-board-bg-option');
    const defaultBg = defaultBgBtn ? defaultBgBtn.style.backgroundImage.replace('url("','').replace('")','').replace("url('",'').replace("')",'') : '';
    
    // Get the first color option as default
    const defaultColorBtn = document.querySelector('.modal-board-color-option');
    const defaultColor = defaultColorBtn ? defaultColorBtn.getAttribute('data-color') : '#FFA800';

    return {
        title,
        background: bgBtn ? bgBtn.style.backgroundImage.replace('url("','').replace('")','').replace("url('",'').replace("')",'') : defaultBg,
        color: colorBtn ? colorBtn.getAttribute('data-color') : defaultColor
    };
}
